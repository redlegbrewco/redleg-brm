...
var SHEETS = {
  ...
  PACKAGING_CONFIG: 'Packaging Config'
};
...
// ═══════════════════════════════════════════════════════════════════════════════
// PHASE 1: BREWER'S SHEET CONSOLIDATION - DATA LAYER FOUNDATION
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Setup Packaging Config sheet - Phase 1
 * Creates sheet with packaging material requirements per package type
 */
function setupPackagingConfigSheet() {
  try {
    var ss = getBrmSpreadsheet();
    var sheet = ss.getSheetByName(SHEETS.PACKAGING_CONFIG);
    var sheetWasNew = false;
    ...
    if (sheet) {
      ...
      try {
        ...
        if (data && data.length > 1) {
          // Sheet exists and has data - check if it's just headers or has actual data
          var hasData = false;
          for (var i = 1; i < data.length; i++) {
          ...
          if (hasData) {
            return { success: true, message: 'Packaging Config sheet already exists with data' };
          }
          // Sheet exists but only has headers or is empty - will populate below
        }
      } catch (e) {
        // If getDataRange fails (empty sheet), continue to populate
        Logger.log('Sheet exists but may be empty: ' + e.toString());
      }
    } else {
      // Create new sheet
      sheet = ss.insertSheet(SHEETS.PACKAGING_CONFIG);
      sheetWasNew = true;
    }
    ...
    // Column widths
    sheet.setColumnWidth(1, 150);  // Package Type
    sheet.setColumnWidth(2, 200);  // Material Name
    sheet.setColumnWidth(3, 120);  // Quantity Per Unit
    sheet.setColumnWidth(4, 80);   // Unit
    
    // Default data
    var defaultData = [
      ['1/2 BBL Keg', 'Keg Cap', 1, 'ea'],
      ['1/2 BBL Keg', 'Keg Collar', 1, 'ea'],
      ['1/6 BBL Keg', 'Keg Cap', 1, 'ea'],
      ['1/6 BBL Keg', 'Keg Collar', 1, 'ea'],
      ['12oz Case', '12oz Can', 24, 'ea'],
      ['12oz Case', '12oz Case Tray', 1, 'ea'],
      ['12oz Case', '12oz 6-Pack Carrier', 4, 'ea'],
      ['12oz Case', '12oz Lid', 24, 'ea'],
      ['16oz Case', '16oz Can', 24, 'ea'],
      ['16oz Case', '16oz Case Tray', 1, 'ea'],
      ['16oz Case', '16oz 6-Pack Carrier', 4, 'ea'],
      ['16oz Case', '16oz Lid', 24, 'ea']
    ];
    
    // Check if we need to add data
    var needsData = sheetWasNew; // New sheet definitely needs data
    if (!sheetWasNew) {
      // Existing sheet - check if it has data beyond headers
      try {
        var data = sheet.getDataRange().getValues();
        if (!data || data.length <= 1) {
          needsData = true;
        } else {
          // Check if any row has actual data (not just empty cells)
          var hasData = false;
          for (var i = 1; i < data.length; i++) {
            if (data[i][0] && data[i][0].toString().trim() !== '') {
              hasData = true;
        ...
        // If we can't read data, assume we need to add it
        needsData = true;
      }
    }
    ...
    if (needsData) {
      ...
      Logger.log('Added default data to Packaging Config sheet');
    }
    
    Logger.log('Packaging Config sheet setup complete');
    return { success: true, message: 'Packaging Config sheet ready' };
    
  } catch (e) {
    Logger.log('Error setting up Packaging Config sheet: ' + e.toString());
    return { success: false, error: e.toString() };
  }
}

/**
 * Get packaging materials config - Phase 1
 * Returns array of {packageType, materialName, quantityPerUnit, unit}
 */
function getPackagingMaterialsConfig() {
  try {
    var ss = getBrmSpreadsheet();
    var sheet = ss.getSheetByName(SHEETS.PACKAGING_CONFIG);
    ...
    if (!sheet) {
      // Try to create it
      var setupResult = setupPackagingConfigSheet();
      if (!setupResult.success) {
        return { success: false, error: 'Could not create Packaging Config sheet', materials: [] };
      }
      sheet = ss.getSheetByName(SHEETS.PACKAGING_CONFIG);
    }
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      materials.push({
        packageType: data[i][0] || '',
        materialName: data[i][1] || '',
        quantityPerUnit: parseFloat(data[i][2]) || 0,
        unit: data[i][3] || 'ea'
      });
    }
    ...
    Logger.log('Error getting packaging materials config: ' + e.toString());
    return { success: false, error: e.toString(), materials: [] };
  }
}

/**
 * Calculate packaging materials needed - Phase 1
 * @param {number} halfBBL - Quantity of 1/2 BBL kegs
 * @param {number} sixthBBL - Quantity of 1/6 BBL kegs
 * @param {number} cases12oz - Quantity of 12oz cases
 * @param {number} cases16oz - Quantity of 16oz cases
 * @returns {Object} Array of {materialName, totalQuantity, unit}
 */
function calculatePackagingMaterials(halfBBL, sixthBBL, cases12oz, cases16oz) {
  try {
    var configResult = getPackagingMaterialsConfig();
    if (!configResult.success || !configResult.materials) {
      return { success: false, error: 'Could not load packaging config', materials: [] };
    }
    
    var packageBreakdown = {
      '1/2 BBL Keg': halfBBL || 0,
      '1/6 BBL Keg': sixthBBL || 0,
      '12oz Case': cases12oz || 0,
      '16oz Case': cases16oz || 0
    };
    
    var materialTotals = {};
    
    // Calculate totals for each material
    configResult.materials.forEach(function(item) {
      var packageType = item.packageType;
      var qty = packageBreakdown[packageType] || 0;
      
      if (qty > 0) {
        var materialKey = item.materialName + '|' + item.unit;
        ...
        materialTotals[materialKey].totalQuantity += qty * item.quantityPerUnit;
      }
    });
    
    // Convert to array
    var materials = [];
    for (var key in materialTotals) {
      materials.push(materialTotals[key]);
    }
    ...
    Logger.log('Error calculating packaging materials: ' + e.toString());
    return { success: false, error: e.toString(), materials: [] };
  }
}

/**
 * Get batch workflow state - Phase 1
 * Returns workflow completion status for a batch
 * @param {string} batchNumber - Batch number
 * @returns {Object} Workflow state object
 */
function getBatchWorkflowState(batchNumber) {
  try {
    var ss = getBrmSpreadsheet();
    var batchSheet = ss.getSheetByName(SHEETS.BATCH_LOG);
    ...
    // Get batch data - columns may vary, so we'll check what exists
    var status = data[batchRow - 1][10] || ''; // K: Status
    var currentVessel = data[batchRow - 1][28] || ''; // AC: Current Vessel
    
    // Check for new workflow columns (AD onwards)
    var transferToFVDate = batchRow > 0 && data[batchRow - 1].length > 29 ? data[batchRow - 1][29] : '';
    var transferToFVBy = batchRow > 0 && data[batchRow - 1].length > 30 ? data[batchRow - 1][30] : '';
    var fvVessel = batchRow > 0 && data[batchRow - 1].length > 31 ? data[batchRow - 1][31] : '';
    var crashDate = batchRow > 0 && data[batchRow - 1].length > 32 ? data[batchRow - 1][32] : '';
    var transferToBTDate = batchRow > 0 && data[batchRow - 1].length > 33 ? data[batchRow - 1][33] : '';
    var transferToBTBy = batchRow > 0 && data[batchRow - 1].length > 34 ? data[batchRow - 1][34] : '';
    var btVessel = batchRow > 0 && data[batchRow - 1].length > 35 ? data[batchRow - 1][35] : '';
    var pkgCompleteDate = batchRow > 0 && data[batchRow - 1].length > 36 ? data[batchRow - 1][36] : '';
    var pkgCompleteBy = batchRow > 0 && data[batchRow - 1].length > 37 ? data[batchRow - 1][37] : '';
    
    // Determine workflow state
    var brewLaborComplete = false; // Will be determined by Batch Details entries
    var transferredToFV = transferToFVDate !== '' && transferToFVDate !== null;
    var transferredToBT = transferToBTDate !== '' && transferToBTDate !== null;
    var packagingComplete = pkgCompleteDate !== '' && pkgCompleteDate !== null;
    var sentIt = status === 'Packaged';
    
    // Check Batch Details for brew labor entries
    var detailsSheet = ss.getSheetByName('Batch Details');
    if (detailsSheet) {
      var detailsData = detailsSheet.getDataRange().getValues();
      for (var i = 1; i < detailsData.length; i++) {
        if (detailsData[i][0] && detailsData[i][0].toString() === batchNumber) {
          var entryType = detailsData[i][3] || '';
          if (entryType === 'Brew Labor' || entryType === 'Transfer') {
            brewLaborComplete = true;
            break;
          }
        }
      }
    ...
    return serializeForHtml({
      ...
      batchNumber: batchNumber,
      workflowState: {
        brewLaborComplete: brewLaborComplete,
        transferredToFV: transferredToFV,
        transferredToBT: transferredToBT,
        packagingComplete: packagingComplete,
        sentIt: sentIt
      },
      dates: {
        transferToFVDate: transferToFVDate,
        transferToBTDate: transferToBTDate,
        packagingCompleteDate: pkgCompleteDate
      },
      vessels: {
        fvVessel: fvVessel || currentVessel,
        btVessel: btVessel || currentVessel
      },
      status: status
    });
    
  } catch (e) {
    Logger.log('Error getting batch workflow state: ' + e.toString());
    return { success: false, error: e.toString() };
  }
}

/**
 * Ensure Batch Log has required workflow columns - Phase 1
 * Adds columns if they don't exist
 * Batch Log structure: Row 1 = Title, Row 9 = Headers, Row 10+ = Data
 */
function ensureBatchLogWorkflowColumns() {
  try {
    var ss = getBrmSpreadsheet();
    var batchSheet = ss.getSheetByName(SHEETS.BATCH_LOG);
    ...
    // Batch Log structure: Row 9 is the header row (Row 1 is title, data starts Row 10)
    var headerRow = 9;
    var lastCol = batchSheet.getLastColumn();
    
    // First, clean up any mistakenly added headers in Row 1 (columns AD-AL)
    try {
      var row1Data = batchSheet.getRange(1, 30, 1, 9).getValues()[0]; // Columns AD-AL (30-38)
      var needsCleanup = false;
      for (var i = 0; i < row1Data.length; i++) {
        var cellValue = row1Data[i];
        if (cellValue && cellValue.toString().trim() !== '') {
          // Check if it's one of our workflow column headers
          var workflowHeaders = [
            'Transfer to FV Date', 'Transfer to FV By', 'FV/LT Vessel',
            'Crash Date', 'Transfer to BT Date', 'Transfer to BT By',
            'BT Vessel', 'Packaging Complete Date', 'Packaging Complete By'
          ];
          if (workflowHeaders.indexOf(cellValue.toString().trim()) !== -1) {
            needsCleanup = true;
            break;
          }
        }
      ...
      if (needsCleanup) {
        // Clear the mistakenly added headers from Row 1
        batchSheet.getRange(1, 30, 1, 9).clearContent();
        Logger.log('Cleaned up mistakenly added headers from Row 1');
      }
    } catch (e) {
      Logger.log('Note: Could not check Row 1 for cleanup: ' + e.toString());
    }
    
    // Get header row data (Row 9)
    var headerData = batchSheet.getRange(headerRow, 1, 1, Math.max(lastCol, 37)).getValues()[0];
    
    // Required workflow columns starting at AD (column 30)
    var requiredColumns = [
      { name: 'Transfer to FV Date', colIndex: 30 },      // AD
      { name: 'Transfer to FV By', colIndex: 31 },        // AE
      { name: 'FV/LT Vessel', colIndex: 32 },             // AF
      { name: 'Crash Date', colIndex: 33 },               // AG
      { name: 'Transfer to BT Date', colIndex: 34 },      // AH
      { name: 'Transfer to BT By', colIndex: 35 },       // AI
      { name: 'BT Vessel', colIndex: 36 },                // AJ
      { name: 'Packaging Complete Date', colIndex: 37 },  // AK
      { name: 'Packaging Complete By', colIndex: 38 }    // AL
    ];
    ...
    if (lastCol < 38) {
      // Need to add columns to reach column AL (38)
      for (var c = lastCol + 1; c <= 38; c++) {
    ...
    requiredColumns.forEach(function(col) {
      // Check if header exists in Row 9 at the correct column
      ...
      if (existingHeader && existingHeader.toString().trim() === col.name) {
        ...
        // Also check if header exists anywhere in Row 9 (in case columns shifted)
      ...
      if (!headerExists) {
        // Add column header to Row 9 at the correct column
    ...
    if (columnsAdded.length > 0) {
      Logger.log('Added Batch Log workflow columns: ' + columnsAdded.join(', '));
      return { 
        success: true, 
        message: 'Added ' + columnsAdded.length + ' workflow columns to Row 9',
        columnsAdded: columnsAdded
      };
    } else {
      return { 
        success: true, 
        message: 'All required workflow columns already exist in Row 9',
        columnsAdded: []
      };
    }
    
  } catch (e) {
    Logger.log('Error ensuring Batch Log columns: ' + e.toString());
    return { success: false, error: e.toString() };
  }
}

/**
 * Categorize recipe ingredients by depletion stage - Phase 1
 * Returns ingredients grouped by when they should deplete
 * @param {string} recipeName - Recipe name
 * @returns {Object} Categorized ingredients
 */
function categorizeRecipeIngredientsByDepletionStage(recipeName) {
  try {
    var ss = getBrmSpreadsheet();
    var recipeSheet = ss.getSheetByName(SHEETS.RECIPES);
    var ingredientsSheet = ss.getSheetByName(SHEETS.RECIPE_INGREDIENTS);
    
    if (!recipeSheet || !ingredientsSheet) {
      return { success: false, error: 'Recipe or Recipe Ingredients sheet not found' };
    }
    ...
    var recipeRow = -1;
    for (var i = 1; i < recipeData.length; i++) {
      if (recipeData[i][0] && recipeData[i][0].toString() === recipeName) {
        recipeRow = i;
        break;
      }
    }
    ...
    // Get ingredients for this recipe
    var ingData = ingredientsSheet.getDataRange().getValues();
    var mashBoilIngredients = []; // Deplete at Transfer to FV
    var cellarIngredients = [];   // Deplete when task marked complete
    
    for (var i = 1; i < ingData.length; i++) {
      if (ingData[i][0] && ingData[i][0].toString() === recipeName) {
        var category = (ingData[i][1] || '').toString().toLowerCase();
        var ingredient = ingData[i][2] || '';
        var amount = parseFloat(ingData[i][3]) || 0;
        var uom = ingData[i][4] || 'lb';
        var stage = (ingData[i][5] || '').toString().toLowerCase(); // Depletion Stage column if exists
        ...
        var ingObj = {
          ...
          amount: amount,
          uom: uom,
          depletionStage: stage || 'auto'
        };
        
        // Categorize by depletion stage or category
        if (stage === 'cellar' || stage === 'addition' || category === 'dry hops' || 
            category === 'finings' || category === 'adjuncts') {
          cellarIngredients.push(ingObj);
        } else {
          // Default: mash/boil ingredients (grains, boil hops, yeast, water treatments)
          mashBoilIngredients.push(ingObj);
        }
      }
    }
    ...
    return serializeForHtml({
      success: true,
      recipeName: recipeName,
      mashBoilIngredients: mashBoilIngredients,  // Deplete at Transfer to FV
      cellarIngredients: cellarIngredients        // Deplete when task marked complete
    });
    
  } catch (e) {
    Logger.log('Error categorizing recipe ingredients: ' + e.toString());
    return { success: false, error: e.toString() };
  }
}

/**
 * Initialize Phase 1 setup - Run this once to set up all Phase 1 infrastructure
 * Creates sheets and adds columns as needed
 */
function initializePhase1Setup() {
  try {
    var results = {
